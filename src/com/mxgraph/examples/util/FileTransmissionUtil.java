 ...
 *
 */
public class FileTransmissionUtil {
	
	private static Log log = LogFactory.getLog(FileTransmissionUtil.class);
	 ...
	 * @param password 密码
	 */
	public static void transfor(String sourcePath,String targetPath,String targetIP,String username,String password){
		ChannelSftp sftp = null;
		Session session = null;
		try {
			session = FileTransmissionUtil.connect(targetIP, null, username, password);
			Channel channel = session.openChannel("sftp");
			channel.connect();
			sftp = (ChannelSftp) channel;
			File srcfile = new File(sourcePath);
			FileTransmissionUtil.upload(targetPath, srcfile.getAbsolutePath(), sftp);
		} catch (Exception e) {
			e.printStackTrace();
		}finally{
	...
	public static void main(String[] args){
		...
		Session session = null;
		try {
			session = FileTransmissionUtil.connect("10.109.253.71", null, "spark", "123456");
			Channel channel = session.openChannel("sftp");
			channel.connect();
			sftp = (ChannelSftp) channel;
			File srcfile = new File("/home/zhou/mydb.sql");
			FileTransmissionUtil.upload("/home/spark/", srcfile.getAbsolutePath(), sftp);
		} catch (Exception e) {
			e.printStackTrace();
		}finally{
			if(sftp != null)sftp.disconnect();
