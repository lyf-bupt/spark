...
Editor = function()
{
	...
	this.webserviceGraph = new Graph();
	...
	// Updates modified state if graph changes
	this.webserviceGraph.getModel().addListener(mxEvent.CHANGE, mxUtils.bind(this, function()
	{
		this.modified = true;
	}));
	...
	// Sets persistent graph state defaults
	this.webserviceGraph.resetViewOnRootChange = false;
	this.webserviceGraph.scrollbars = true;
	this.webserviceGraph.background = null;
};
 ...
 * Sets the XML node for the current diagram.
 */
Editor.prototype.setWebGraphXml = function(node)
{
	var dec = new mxCodec(node.ownerDocument);
	
	if (node.nodeName == 'mxGraphModel')
	{
		this.webserviceGraph.view.scale = Number(node.getAttribute('scale') || 1);
		this.webserviceGraph.gridEnabled = node.getAttribute('grid') != '0';
		this.webserviceGraph.graphHandler.guidesEnabled = node.getAttribute('guides') != '0';
		this.webserviceGraph.setTooltips(node.getAttribute('tooltips') != '0');
		this.webserviceGraph.setConnectable(node.getAttribute('connect') != '0');
		this.webserviceGraph.foldingEnabled = node.getAttribute('fold') != '0';
		this.webserviceGraph.scrollbars = node.getAttribute('scrollbars') != '0';
		
		if (!this.webserviceGraph.scrollbars)
		{
			this.webserviceGraph.container.scrollLeft = 0;
			this.webserviceGraph.container.scrollTop = 0;
			this.webserviceGraph.view.translate.x = Number(node.getAttribute('dx') || 0);
			this.webserviceGraph.view.translate.y = Number(node.getAttribute('dy') || 0);
		}

		this.webserviceGraph.pageVisible = node.getAttribute('page') == '1';
		this.webserviceGraph.pageBreaksVisible = this.webserviceGraph.pageVisible; 
		this.webserviceGraph.preferPageSize = this.webserviceGraph.pageBreaksVisible;
		...
		if (ps != null)
		{
			this.webserviceGraph.pageScale = ps;
		}
		else
		{
			this.webserviceGraph.pageScale = 1.5;
		}
		...
		if (pw != null && ph != null)
		{
			this.webserviceGraph.pageFormat = new mxRectangle(0, 0, parseFloat(pw), parseFloat(ph));
		}
		...
		if (bg != null && bg.length > 0)
		{
			this.webserviceGraph.background = bg;
		}
		
		dec.decode(node, this.webserviceGraph.getModel());
		this.updateGraphComponents();
	}
};
 ...
 * Returns the XML node that represents the current diagram.
 */
Editor.prototype.getWebGraphXml = function()
{
	var enc = new mxCodec(mxUtils.createXmlDocument());
	var node = enc.encode(this.webserviceGraph.getModel());

	if (this.webserviceGraph.view.translate.x != 0 || this.webserviceGraph.view.translate.y != 0)
	{
		node.setAttribute('dx', Math.round(this.webserviceGraph.view.translate.x * 100) / 100);
		node.setAttribute('dy', Math.round(this.webserviceGraph.view.translate.y * 100) / 100);
	}
	
	if (this.webserviceGraph.view.scale != 1)
	{
		node.setAttribute('scale', Math.round(this.webserviceGraph.view.scale * 1000) / 1000);
	}
	
	node.setAttribute('grid', (this.webserviceGraph.isGridEnabled()) ? '1' : '0');
	node.setAttribute('guides', (this.webserviceGraph.graphHandler.guidesEnabled) ? '1' : '0');
	node.setAttribute('guides', (this.webserviceGraph.graphHandler.guidesEnabled) ? '1' : '0');
	node.setAttribute('tooltips', (this.webserviceGraph.tooltipHandler.isEnabled()) ? '1' : '0');
	node.setAttribute('connect', (this.webserviceGraph.connectionHandler.isEnabled()) ? '1' : '0');	
	node.setAttribute('fold', (this.webserviceGraph.foldingEnabled) ? '1' : '0');
	node.setAttribute('page', (this.webserviceGraph.pageVisible) ? '1' : '0');
	node.setAttribute('pageScale', this.webserviceGraph.pageScale);
	node.setAttribute('pageWidth', this.webserviceGraph.pageFormat.width);
	node.setAttribute('pageHeight', this.webserviceGraph.pageFormat.height);
	...
	if (this.webserviceGraph.background != null)
	{
		node.setAttribute('background', this.webserviceGraph.background);
	}
...
/**
 * Keeps the webserviceGraph container in sync with the persistent webserviceGraph state
 */
Editor.prototype.updateWebGraphComponents = function()
{
	var webserviceGraph = this.webserviceGraph;
	var outline = this.outline;
	
	if (webserviceGraph.container != null && outline.outline.container != null)
	{
		if (webserviceGraph.background != null)
		{
			if (webserviceGraph.background == 'none')
			{
				webserviceGraph.container.style.backgroundColor = 'transparent';
			}
			else
			{
				if (webserviceGraph.view.backgroundPageShape != null)
				{
					webserviceGraph.view.backgroundPageShape.fill = webserviceGraph.background;
					webserviceGraph.view.backgroundPageShape.reconfigure();
				}
				
				webserviceGraph.container.style.backgroundColor = webserviceGraph.background;
			}
		}
		else
		{
			webserviceGraph.container.style.backgroundColor = '';
		}

		if (webserviceGraph.pageVisible)
		{
			webserviceGraph.container.style.backgroundColor = '#ebebeb';
			webserviceGraph.container.style.borderStyle = 'solid';
			webserviceGraph.container.style.borderColor = '#e5e5e5';
			webserviceGraph.container.style.borderTopWidth = '1px';
			webserviceGraph.container.style.borderLeftWidth = '1px';
			webserviceGraph.container.style.borderRightWidth = '0px';
			webserviceGraph.container.style.borderBottomWidth = '0px';
		}
		else
		{
			webserviceGraph.container.style.border = '';
		}
		
		outline.outline.container.style.backgroundColor = webserviceGraph.container.style.backgroundColor;

		if (outline.outline.pageVisible != webserviceGraph.pageVisible ||
			outline.outline.pageScale != webserviceGraph.pageScale)
		{
			outline.outline.pageScale = webserviceGraph.pageScale;
			outline.outline.pageVisible = webserviceGraph.pageVisible;
			outline.outline.view.validate();
		}
		
		if (webserviceGraph.scrollbars && webserviceGraph.container.style.overflow == 'hidden' && !touchStyle)
		{
			webserviceGraph.container.style.overflow = 'auto';
		}
		else if (!webserviceGraph.scrollbars || touchStyle)
		{
			webserviceGraph.container.style.overflow = 'hidden';
		}
		
		// Transparent.gif is a workaround for focus repaint problems in IE
		var noBackground = (mxClient.IS_IE && document.documentMode >= 9) ? 'url(' + this.transparentImage + ')' : 'none';
		webserviceGraph.container.style.backgroundImage = (!webserviceGraph.pageVisible && webserviceGraph.isGridEnabled()) ? 'url(' + this.gridImage + ')' : noBackground;

		if (webserviceGraph.view.backgroundPageShape != null)
		{
			webserviceGraph.view.backgroundPageShape.node.style.backgroundImage = (this.webserviceGraph.isGridEnabled()) ? 'url(' + this.gridImage + ')' : 'none';
		}
	}
};
