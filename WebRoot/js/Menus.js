...
Menus.prototype.init = function()
{
	...
	{
		this.addMenuItems(menu, ['new', 'open', '-', 'save', 'saveAs', '-', 'export','generate', '-', 'import', 'editFile', '-', 'pageSetup', 'print'], parent);
	})));
	...
	{
		this.addMenuItems(menu, ['help', '-', 'about','-','wsBench']);
	})));
};
...
Menus.prototype.createPopupMenu = function(menu, cell, evt)
{
	...
	if (graph.getSelectionCount() > 0)
	{
		...
		if (graph.getSelectionCount() == 1)
		{
			...
			if("ellipse;shape=doubleEllipse"==graph.getSelectionCell().getStyle()){
				menu.addSeparator();
				this.addMenuItems(menu, ['outputModel']);
			}else if("ellipse"==graph.getSelectionCell().getStyle()){
				menu.addSeparator();
				this.addMenuItems(menu, ['source']);
			}else if("ellipse;shape=cloud"==graph.getSelectionCell().getStyle()){
				menu.addSeparator();
				this.addMenuItems(menu, ['webServiceDF']);
			}else if("shape=xor"==graph.getSelectionCell().getStyle()){
				menu.addSeparator();
				this.addMenuItems(menu, ['filter']);
			}else if("rhombus"==graph.getSelectionCell().getStyle()){
				menu.addSeparator();
				this.addMenuItems(menu, ['map']);
			}
		}
	}
	else
	...
	if (funct != null)
	{
		...
		var clickHandler = mxUtils.bind(this, function(evt)
		{
			if (show && elt.enabled == null || elt.enabled)
			{
				...
				menu.popup(elt.offsetLeft + 40, elt.offsetTop + elt.offsetHeight + 39, null, evt);
